...
class SecureStorage: CDVPlugin {
    ...
    func get(command: CDVInvokedUrlCommand) {
        ...
        guard let service = command.argument(at: 0) as? String, let key = command.argument(at: 1) as? String else { return }
    ...
    func set(command: CDVInvokedUrlCommand) {
        ...
        guard let service = command.argument(at: 0) as? String, let key = command.argument(at: 1) as? String, let value = command.argument(at: 2) as? String, let valueData = value.data(using: .utf8) else { return }
        self.commandDelegate.run { [weak self] in
            guard let self = self else { return }
            self.plugin?.save(service: service, account: key, data: valueData)
        }
    }
    ...
    func keys(command: CDVInvokedUrlCommand) {
        ...
        guard let service = command.argument(at: 0) as? String else { return }
        self.commandDelegate.run { [weak self] in
            guard let self = self else { return }
